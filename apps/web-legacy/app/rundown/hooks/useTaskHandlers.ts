...
export function useTaskHandlers() {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (over && active.id !== over.id) {
        ...
        const updatedTasks = [...otherTasks, ...newDayTasks.map((task, index) => ({ ...task, order: index }))];
  ...
  const handleStatusChange = useCallback(
    async (taskId: string, newStatus: TaskStatus) => {
      ...
      if (newStatus === 'ongoing') {
        ...
        updatedCurrentDayTasks = updatedCurrentDayTasks.map((task) =>
          task.status === 'ongoing' && task._id !== taskId ? { ...task, status: 'paused' } : task,
        );
      }
  ...
  const handleTaskResize = useCallback(
    async (id: string, size: number) => {
      ...
      const otherTasksMinutes = currentDayTasks.filter((t) => t._id !== id).reduce((acc, t) => acc + t.size * 15, 0);
