...
## Speckit agent metadata (from `.github/agents`)

Additional Copilot agent frontmatter preserved for reference:
...
- This context note:
  > _"Product Forge context: This spec is backed by exhaustive research. Key integration points are documented in `research/codebase-analysis.md`. User journeys and wireframes are in `product-spec/`. The plan should align with the product-spec feature breakdown and cover all Must Have user stories."_
...
Ask: _"Technical plan created. Cross-validation: {N} checks passed, {N} warnings, {N} issues. Approve plan and move to Task breakdown?"_
...
- FEATURE_DIR
- Note: _"Ensure tasks are decomposed enough to stay within safe implementation size. Reference product-spec/product-spec.md for acceptance criteria that each task group must satisfy. Tasks should be grouped by the feature breakdown sections in product-spec.md."_
...
Ask: _"Task breakdown approved? Begin implementation?"_
...
> _"Product Forge implementation: After completion, run `/speckit.product-forge.verify-full` to perform a full traceability check from code back to the original product spec and research artifacts."_
...
> _"Check `{FEATURE_DIR}/product-spec/product-spec.md § {section}` — this was defined in the product spec."_
